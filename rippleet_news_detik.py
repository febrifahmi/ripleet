...
This code collects data from detik news online portal and index it to the Elasticsearch 
'''
...
urltoparse = "http://rss.detik.com/index.php/detikcom_nasional"
...
for item in artikel_container:
	# if isinstance(item.find('title'), bs4.CData):
	...
	try:
		...
		konten_container = content_soup.find('div', class_='detail_wrap').get_text()
		judul_container = str(content_soup.find('div', class_='jdl'))
		item_judul = re.sub(r"<h1>|</h1>","",re.search(r'<h1>.+</h1>',judul_container).group(0))
